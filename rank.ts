...
export class Rank extends Command {
    ...
    async run(bot: DiscordClientWrapper, msg: Discord.Message, params: string[], perms: number) {
        ...
        checkingParametersMsg.edit(`Je cherche des infos sur: ${username}`)
            .then(async (message: Discord.Message) => {
                const id: string = await pubgService.getCharacterID(username, region);
                if (!id) {
                    message.edit(`Je ne trouve aucun ${username} sur la région ${region}. `);
                    return;
                }
                ...
                if (soloData) {
                    ...
                    embed.addField('Solo Status', 'Le joueur a fait aucun match en `Solo`', false);
                }
                if (duoData) {
                    ...
                    embed.addBlankField(false);
                    embed.addField('Duo Status', 'Le joueur a fait aucun match en `Duo`', false);
                }
                if (squadData) {
                    ...
                    embed.addBlankField(false);
                    embed.addField('Squad Stats', 'Le joueur a fait aucun match en `Squad` ', false);
                }
                message.edit({ embed });
            });
    };
    ...
    if (playerData.rank > 2000) return 
